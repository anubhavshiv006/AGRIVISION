  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    for (const model of modelsToTry) {
      try {
        ...
        // Model not found or unavailable -> instantly try next model in the list
        ...
        // Rate limit / Quota -> wait and retry with next models (sometimes switching models bypasses quota)
        if (errMsg.includes("429") || errMsg.includes("RESOURCE_EXHAUSTED") || errMsg.includes("quota")) {
          console.warn(\`[RateLimit] Hit quota on \${model}. Switching models...\`);
          continue; 
        }

        // For 503 Service Unavailable, break to retry outer loop
        if (errMsg.includes("503") || errMsg.includes("overloaded")) {
          console.warn(\`[Overload] Service overloaded. Waiting...\`);
          break;
        }
        
        // Other errors (e.g. invalid request) -> throw immediately
        throw error;
      }
    }
    
    // Wait before next retry loop if we didn't succeed
    if (attempt < retries) {
